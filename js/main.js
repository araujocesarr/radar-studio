...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function onScroll() {
    ...
    sections.forEach((section) => {
      ...
      if (inViewport && !isPageEnd) {
        document
          .querySelector(`[data-menuanchor="${id}"]`)
  ...
  function initPaging() {
    if (isTablet() && isInited) {
      ...
      [...document.querySelectorAll(".header__menu-item")].forEach(
        (element) => {
      ...
      !isTablet() &&
      !isInited &&
      document.getElementById("pagepiling")
    ) {
      isInited = true;
      ...
      function setActiveMenu(index) {
        ...
        document
          .querySelector(`[data-menuanchor="${anchor}"]`)
          .classList.add("active");
      }
      ...
      function animationActiveSection(index, nextIndex) {
        ...
        const nextActiveSection = document.querySelector(
          `.section-${nextAnchor}`
        );
  ...
  setTimeout(disabledPreloader, 50);
